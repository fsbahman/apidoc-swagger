...
function createApidocSwagger(options) {
    ...
    try {
        ...
        if (app.options.useApidocOutput == true){
            api = {
                data   : fs.readFileSync(app.options.src + 'api_data.json', 'utf8'),
                project: fs.readFileSync(app.options.src + 'api_project.json', 'utf8')
            };
        }
        else{
