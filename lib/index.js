...
function createApidocSwagger(options) {
    ...
    try {
        ...
        apidoc.setPackageInfos(packageInfos);
        ...
        if (app.options.parse !== true){
            ...
            var swaggerData = apidocSwagger.toSwagger(apidocData , projectData);

            swaggerData.host = packageInfos.host || '{{url}}';
            swaggerData.basePath = packageInfos.basePath || '';
            swaggerData.schemes = packageInfos.schemes || ['http'];

            api["swaggerData"] = JSON.stringify(swaggerData);
