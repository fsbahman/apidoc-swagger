...
function createParamsDefinitions(verbs, definitions){
	var result = {
		topLevelParametersRef : null,
		topLevelSuccessRef : null,
		topLevelSuccessRefType : null
...
function createSuccessResults(verbs, definitions){
	return _.mapValues(verbs.success.fields, function(success){
		var defaultObjectName = verbs.name;
		var fieldArrayResult = createFieldArrayDefinitions(success, definitions, verbs.name+"Result", defaultObjectName);
		var schema = {};
		switch (fieldArrayResult.topLevelRefType) {
			case 'Object':
			...
			case "Array":
				...
				break;
			default :
				schema["type"] = fieldArrayResult.topLevelRefType;
		}
...
//	var verbDefinitionResult = createVerbDefinitions(verbs,definitions);
	pathItemObject[verbs.type] = {
		...
		parameters: verbs.parameter?createParameters(
	...
	pathItemObject[verbs.type].responses = createSuccessResults(verbs, definitions);
/*
	if (verbDefinitionResult.topLevelSuccessRef) {
		pathItemObject[verbs.type].responses = {
          "200": {
...
function createPathParameters(verbs, pathKeys) {
	...
	if (verbs.parameter) {
		...
		for (var i = 0; i < verbs.parameter.fields.Parameter.length; i++) {
			...
			var place = "path";
			if (pathKeys.indexOf(field)<0) continue;
			var type = param.type;
			pathItemObject.push({
				name: field,
