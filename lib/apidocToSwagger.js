...
function createVerbDefinitions(verbs, definitions) {
	...
	var defaultObjectName = verbs.name + 'Default';
	var fieldArrayResult = {};
	if (verbs && verbs.parameter && verbs.parameter.fields) {
		fieldArrayResult = createFieldArrayDefinitions(verbs.parameter.fields.Parameter, definitions, verbs.name + 'Parameter', defaultObjectName);		
		result.topLevelParametersRef = fieldArrayResult.topLevelRef;
	};

	if (verbs && verbs.success && verbs.success.fields) {
		fieldArrayResult = createFieldArrayDefinitions(verbs.success.fields["Success 200"], definitions, verbs.name + 'Res', defaultObjectName);		
...
function createFieldArrayDefinitions(fieldArray, definitions, topLevelRef, defaultObjectName) {
	...
	for (var i = 0; i < fieldArray.length; i++) {
		...
		var nestedName = createNestedName(parameter.field, topLevelRef);
		...
		if (i == 0) {
			...
			} else if (parameter.type == "Array" || parameter.type.indexOf("[]") !=-1) {
				objectName = nestedName.propertyName;
				nestedName.propertyName = null;				
		...
		if (nestedName.propertyName) {
			...
			var proName = parameter.field.split(".").pop();
			if(parameter.type.toLowerCase() == "object") {
				prop.$ref = "#/definitions/" + objectName + '.' + proName;
			}
			...
			if(typeIndex !== -1 && typeIndex === (type.length - 2)) {
				...
				prop.items = {
					...
					$ref: "#/definitions/" + objectName + '.' + proName
				};
			}
...
function createNestedName(field, topLevelRef) {
	...
	objectName = (objectName == undefined) ? topLevelRef : topLevelRef + '.' + objectName
	return {
