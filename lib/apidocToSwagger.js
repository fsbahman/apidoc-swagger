...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
	...
	var params = createParameters(verbs, pathKeys);
...
function createVerbDefinitions(verbs, definitions, pathKeys) {
	...
	if (verbs && verbs.parameter && verbs.parameter.fields) {
		...
		// default Parameter group or body group, filter out those inside path
		var fieldArray = _.chain([])
			.concat(verbs.parameter.fields.Parameter || [])
			.concat(verbs.parameter.fields.body || [])
			.filter(function(field) {
				return !_.includes(pathKeys, field.field);
			}).value();
	...
	for (var i = 0; i < fieldArray.length; i++) {
		...
		if (nestedName.propertyName) {
			...
			if(parameter.type == "Object") {
				prop.$ref = "#/definitions/" + defaultObjectName + "." + parameter.field;
			}
...
function createGetDeleteOutput(verbs, definitions, pathKeys) {
	...
	pathItemObject[verbs.type] = {
		...
		parameters: createParameters(verbs, pathKeys)
	}
	if (verbDefinitionResult.topLevelSuccessRef) {
		pathItemObject[verbs.type].responses = {
          "200": {
 ...
 * @returns {Array}
 */
function createParameters(verbs, pathKeys) {
	pathKeys = pathKeys || [];
